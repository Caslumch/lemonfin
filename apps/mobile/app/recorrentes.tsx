  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", paddingVertical: 12, borderTopWidth: first ? 0 : 1, borderTopColor: palette.border }}>
      <Pressable onPress={onEdit} style={{ flexDirection: "row", alignItems: "center", gap: 11, flex: 1, opacity: item.active ? 1 : 0.5 }}>
        <View style={{ width: 40, height: 40, borderRadius: radii.md, backgroundColor: palette.muted, alignItems: "center", justifyContent: "center" }}>
          <Ionicons name={categoryIonicon(item.category?.name, item.category?.icon)} size={19} color={palette.text} />
        </View>
        <View style={{ flex: 1 }}>
